...
class Grid:
    ...
    def __getitem__(self, coordinates):
        ...
        if len(coordinates) != self.dimensionality:
            raise ValueError("Expected %d coordinates." % self.dimensionality)
...
class Game:
    ...
    def makeMove(self, coordinates):
        if coordinates not in self.state.getLegalActions():
            return False
    ...
    def getWinner(self):
        ...
        if player1 and player2:
            raise Exception("Game state is invalid -- both players have won.")
